...
trait ReportAttributes { _: AttributeDef ⇒
  val full = attribute("full", F.stringFmt, "Full content of the report", O.readonly)
  val summary = attribute("summary", F.stringFmt, "Summary of the report", O.readonly)
  val operations = attribute("operations", F.stringFmt, "Update operations applied at the end of the job", "[]", O.unaudited)
}
