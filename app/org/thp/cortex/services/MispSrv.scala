...
class MispSrv @Inject() (
    ...
    job.status() match {
      case JobStatus.Success ⇒
        for {
          ...
          artifacts ← jobSrv.findArtifacts(userId, job.id, QueryDSL.any, Some("all"), Nil)._1
  ...
  private def toArtifact(mispType: String, data: String): Either[String, Attachment] = {
    mispType2dataType(mispType) match {
      ...
      case "file" ⇒
        val FAttachment = attachmentSrv.save("noname", "application/octet-stream", Base64.decodeBase64(data)).map { a ⇒ a }
