...
export default class OrganizationUsersController extends PageController {
  constructor($log, UserService, SearchService, localStorageService) {
    'ngInject';

    super('org-users-page');
    ...
    this.state = this.localStorageService.get('org-users-page') || {
      filters: {
        search: null,
        status: []
      },
  ...
  load(page) {
    ...
    this.localStorageService.set('org-users-page', this.state);

    this.SearchService.configure({
      objectType: `organization/${this.organization.id}/user`,
  ...
  newUser() {
    this.UserService.openModal(this.organization, 'create', {})
      .then(() => {
        this.reload();
        //mode;
